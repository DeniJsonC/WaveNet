...
class dw_conv(nn.Module):
    def __init__(self, in_ch, out_ch,kernel_size=3,stride=1,padding=1,scale=2,bias=True,groups=1):
        super().__init__()
        ...
        self.conv2 = nn.Conv2d(in_channels=dw_channel, out_channels=out_ch, kernel_size=kernel_size, stride=stride, padding=padding, groups=out_ch,
                               bias=bias)
    def forward(self,x):
        x=self.conv1(x)
        x=self.conv2(x)
